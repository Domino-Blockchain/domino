...
usage: $0 [cluster_rpc_url] [identity_pubkey]
 ...
 Required arguments:
   cluster_rpc_url  - RPC URL and port for a running Domino cluster (ex: http://34.83.146.144:8899)
...
function parse_stake_account_data_to_file {
  ...
  account_data="$(domino --url "$url" show-stake-account "$account_key")"
...
function display_results_summary {
  ...
  stake_account_balance_total_dom="$(bc <<< "scale=3; $stake_account_balance_total/$LAMPORTS_PER_DOMI")"
  system_account_balance_dom="$(bc <<< "scale=3; $system_account_balance/$LAMPORTS_PER_DOMI")"

  all_account_total_balance="$(bc <<< "scale=3; $system_account_balance+$stake_account_balance_total")"
  all_account_total_balance_dom="$(bc <<< "scale=3; ($system_account_balance+$stake_account_balance_total)/$LAMPORTS_PER_DOMI")"
  ...
  printf "Balance of all STAKE accounts: %'.3f DOMI\n" "$stake_account_balance_total_dom"
  printf "\n"
  printf "Balance of SYSTEM account: %'d lamports\n" "$system_account_balance"
  printf "Balance of SYSTEM account: %'.3f DOMI\n" "$system_account_balance_dom"
  printf "\n"
  printf "Total Balance of ALL accounts: %'d lamports\n" "$all_account_total_balance"
  printf "Total Balance of ALL accounts: %'.3f DOMI\n" "$all_account_total_balance_dom"
  echo "--------------------------------------------------------------------------------------"
}
...
LAMPORTS_PER_DOMI=1000000000 # 1 billion
