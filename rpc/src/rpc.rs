//! The `rpc` module implements the Domino RPC interface.
...
use {
    ...
    domino_faucet::faucet::request_airdrop_transaction,
    domino_gossip::{cluster_info::ClusterInfo, contact_info::ContactInfo},
    domino_ledger::{
        blockstore::Blockstore, blockstore_db::BlockstoreError, get_tmp_ledger_path,
        leader_schedule_cache::LeaderScheduleCache,
    },
    domino_metrics::inc_new_counter_info,
    domino_perf::packet::PACKET_DATA_SIZE,
    domino_runtime::{
    ...
    domino_send_transaction_service::{
...
pub struct JsonRpcRequestProcessor {
    ...
    bigtable_ledger_storage: Option<domino_storage_bigtable::LedgerStorage>,
...
impl JsonRpcRequestProcessor {
    ...
    fn check_bigtable_result<T>(
        &self,
        result: &std::result::Result<T, domino_storage_bigtable::Error>,
    ) -> Result<()>
    where
        T: std::fmt::Debug,
    {
        if result.is_err() {
            let err = result.as_ref().unwrap_err();
            if let domino_storage_bigtable::Error::BlockNotFound(slot) = err {
    ...
    pub fn get_stake_activation(
        ...
        if bank.epoch().saturating_sub(epoch) > domino_sdk::stake_history::MAX_ENTRIES as u64 {
            return Err(Error::invalid_params(format!(
        ...
        let stake_history =
            domino_sdk::account::from_account::<StakeHistory, _>(&stake_history_account)
                .ok_or_else(Error::internal_error)?;
...
pub mod rpc_minimal {
    ...
    pub trait Minimal {
        ...
        // TODO: Refactor `domino-validator wait-for-restart-window` to not require this method, so
    ...
    impl Minimal for MinimalImpl {
        ...
        fn get_version(&self, _: Self::Metadata) -> Result<RpcVersionInfo> {
            ...
            let version = domino_version::Version::default();
            Ok(RpcVersionInfo {
                domino_core: version.to_string(),
            ...
            Ok(meta
                ...
                .map(|leader_schedule| {
                    let mut schedule_by_identity =
                        domino_ledger::leader_schedule_utils::leader_schedule_by_identity(
    ...
    impl BankData for BankDataImpl {
        ...
        fn get_block_production(
            ...
            for identity in slot_leaders {
                ...
                if slot_history.check(slot) == domino_sdk::slot_history::Check::Found {
...
pub fn create_test_transactions_and_populate_blockstore(
    ...
    let success_tx =
        domino_sdk::system_transaction::transfer(mint_keypair, &keypair1.pubkey(), 2, blockhash);
    let success_signature = success_tx.signatures[0];
    let entry_1 = domino_entry::entry::next_entry(&blockhash, 1, vec![success_tx]);
    // Failed transaction, InstructionError
    let ix_error_tx =
        domino_sdk::system_transaction::transfer(keypair2, &keypair3.pubkey(), 10, blockhash);
    let ix_error_signature = ix_error_tx.signatures[0];
    let entry_2 = domino_entry::entry::next_entry(&entry_1.hash, 1, vec![ix_error_tx]);
    // Failed transaction
    let fail_tx = domino_sdk::system_transaction::transfer(
        mint_keypair,
        &keypair2.pubkey(),
        2,
        Hash::default(),
    );
    let entry_3 = domino_entry::entry::next_entry(&entry_2.hash, 1, vec![fail_tx]);
    let entries = vec![entry_1, entry_2, entry_3];

    let shreds = domino_ledger::blockstore::entries_to_test_shreds(
        entries.clone(),
        slot,
        previous_slot,
        true,
    ...
    let _result = domino_ledger::blockstore_processor::process_entries_for_tests(
        &bank,
        entries,
        true,
        Some(
            &domino_ledger::blockstore_processor::TransactionStatusSender {
...
pub mod tests {
    use {
        ...
        domino_client::rpc_filter::{Memcmp, MemcmpEncodedBytes},
        domino_gossip::{contact_info::ContactInfo, socketaddr},
        domino_ledger::{
    ...
    fn test_rpc_request_processor_new() {
        let bob_pubkey = domino_sdk::pubkey::new_rand();
    ...
    fn test_rpc_get_leader_schedule() {
        ...
        .iter()
        {
            ...
            assert_eq!(
                bob_schedule.len(),
                domino_ledger::leader_schedule_utils::leader_schedule(bank.epoch(), &bank)
                    .unwrap()
                    .get_slot_leaders()
                    .len()
            );
    ...
    fn test_rpc_get_account_info() {
        ...
        let address = domino_sdk::pubkey::new_rand();
    ...
    fn test_rpc_get_program_accounts() {
        ...
        let new_program_id = domino_sdk::pubkey::new_rand();
        ...
        let authority = domino_sdk::pubkey::new_rand();
        let instruction = system_instruction::create_nonce_account(
            &alice.pubkey(),
    ...
    fn test_rpc_simulate_transaction() {
        let RpcHandler {
            ...
            bank,
            ..
        } = start_rpc_handler_with_tx(&domino_sdk::pubkey::new_rand());
    ...
    fn test_token_rpcs() {
        ...
        let token_account_pubkey = domino_sdk::pubkey::new_rand();
        ...
        let other_token_account_pubkey = domino_sdk::pubkey::new_rand();
        ...
        let token_with_different_mint_pubkey = domino_sdk::pubkey::new_rand();
        ...
        let token_with_smaller_balance = domino_sdk::pubkey::new_rand();
    ...
    fn test_decode_and_deserialize_too_large_payloads_fail() {
        ...
        let expect58 = Error::invalid_params(format!(
            "encoded domino_sdk::transaction::Transaction too large: {} bytes (max: encoded/raw {}/{})",
        ...
        let expect = Error::invalid_params(format!(
            "encoded domino_sdk::transaction::Transaction too large: {} bytes (max: {} bytes)",
