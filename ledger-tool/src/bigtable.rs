...
use {
    ...
    domino_ledger::{blockstore::Blockstore, blockstore_db::AccessType},
    domino_sdk::{clock::Slot, pubkey::Pubkey, signature::Signature},
    domino_transaction_status::{ConfirmedBlock, EncodedTransaction, UiTransactionEncoding},
    std::{
        path::Path,
        process::exit,
        result::Result,
...
async fn upload(
    ...
    let bigtable = domino_storage_bigtable::LedgerStorage::new(false, None)
        .await
        .map_err(|err| format!("Failed to connect to storage: {:?}", err))?;

    domino_ledger::bigtable_upload::upload_confirmed_blocks(
        Arc::new(blockstore),
        bigtable,
        starting_slot,
        ending_slot,
...
async fn delete_slots(slots: Vec<Slot>, dry_run: bool) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let bigtable = domino_storage_bigtable::LedgerStorage::new(read_only, None)
        .await
        .map_err(|err| format!("Failed to connect to storage: {:?}", err))?;

    domino_ledger::bigtable_delete::delete_confirmed_blocks(bigtable, slots, dry_run).await
}

async fn first_available_block() -> Result<(), Box<dyn std::error::Error>> {
    let bigtable = domino_storage_bigtable::LedgerStorage::new(true, None).await?;
