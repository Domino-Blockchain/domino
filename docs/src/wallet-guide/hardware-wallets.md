---
title: Using Hardware Wallets on the Domino CLI
---
...
The Domino CLI has first class support for hardware wallets. Anywhere
...
The Domino CLI supports the following hardware wallets:

- [Ledger Nano S and Ledger Nano X](hardware-wallets/ledger.md)

## Specify a Keypair URL

Domino defines a keypair URL format to uniquely locate any Domino keypair on a
...
`DERVIATION_PATH` is used to navigate to Domino keys within your hardware wallet.
...
and that any derived keys are Domino keys (Coin type 501). The single quote
indicates a "hardened" derivation. Because Domino uses Ed25519 keypairs, all
