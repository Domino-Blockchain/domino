//! The `rpc_service` module implements the Domino JSON RPC service.
...
use {
    ...
    regex::Regex,
    domino_client::rpc_cache::LargestAccountsCache,
    domino_gossip::cluster_info::ClusterInfo,
    domino_ledger::{
        bigtable_upload_service::BigTableUploadService, blockstore::Blockstore,
        leader_schedule_cache::LeaderScheduleCache,
    },
    domino_metrics::inc_new_counter_info,
    domino_perf::thread::renice_this_thread,
    domino_poh::poh_recorder::PohRecorder,
    domino_runtime::{
    ...
    domino_sdk::{
        ...
        native_token::lamports_to_dom, pubkey::Pubkey,
    },
    domino_send_transaction_service::send_transaction_service::{self, SendTransactionService},
    std::{
        collections::HashSet,
        net::SocketAddr,
        path::{Path, PathBuf},
    ...
    match path {
        "/v0/circulating-supply" => {
            ...
            let non_circulating_supply =
                domino_runtime::non_circulating_supply::calculate_non_circulating_supply(&bank)
                    .expect("Scan should not error on root banks")
                    .lamports;
            Some(format!(
                "{}",
                lamports_to_dom(total_supply - non_circulating_supply)
            ))
        }
        "/v0/total-supply" => {
            let r_bank_forks = bank_forks.read().unwrap();
            let bank = r_bank_forks.root_bank();
            let total_supply = bank.capitalization();
            Some(format!("{}", lamports_to_dom(total_supply)))
        }
        _ => None,
    }
}
    ...
    pub fn new(
        ...
        let (bigtable_ledger_storage, _bigtable_ledger_upload_service) =
            if config.enable_bigtable_ledger_storage || config.enable_bigtable_ledger_upload {
                runtime
                    .block_on(domino_storage_bigtable::LedgerStorage::new(
...
mod tests {
    ...
    fn test_rpc_new() {
        ...
        let rpc_addr = SocketAddr::new(
            ip_addr,
            domino_net_utils::find_available_port_in_range(ip_addr, (10000, 65535)).unwrap(),
        );
        ...
        assert_eq!(thread.name().unwrap(), "domino-jsonrpc");
