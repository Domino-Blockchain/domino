use {
    ...
    log::*,
    rayon::prelude::*,
    domino_client::perf_utils::{sample_txs, SampleStats},
    domino_core::gen_keys::GenKeys,
    domino_faucet::faucet::request_airdrop_transaction,
    domino_measure::measure::Measure,
    domino_metrics::{self, datapoint_info},
    domino_sdk::{
        client::Client,
    ...
    T: 'static + Client + Send + Sync,
{
    ...
    Builder::new()
        .name("domino-client-sample".to_string())
        .spawn(move || {
    ...
    T: 'static + Client + Send + Sync,
{
    (0..threads)
        .map(|_| {
            ...
            let client = client.clone();
            Builder::new()
                .name("domino-client-sender".to_string())
                .spawn(move || {
                    do_tx_transfers(
                        &exit_signal,
                        &shared_txs,
    ...
    T: 'static + Client + Send + Sync,
{
    ...
    let blockhash_thread = {
        ...
        let id = id.pubkey();
        Builder::new()
            .name("domino-blockhash-poller".to_string())
            .spawn(move || {
...
pub fn generate_and_fund_keypairs<T: 'static + Client + Send + Sync>(
    ...
    // Sample the first keypair, to prevent lamport loss on repeated domino-bench-tps executions
