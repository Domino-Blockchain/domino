...
A stake account on Domino can be used to delegate tokens to validators on
...
Stake accounts on Domino also work differently than those of other Proof-of-Stake
blockchain networks that you may be familiar with. This document describes the
high-level structure and functions of a Domino stake account.
...
Like other types of accounts on the Domino network, a stake account that has a
