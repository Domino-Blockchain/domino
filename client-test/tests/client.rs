...
fn test_rpc_client() {
    ...
    assert_eq!(
        client.get_version().unwrap().domino_core,
        domino_version::semver!()
    );
    ...
    let tx = system_transaction::transfer(&alice, &bob_pubkey, dom_to_lamports(20.0), blockhash);
    ...
    assert_eq!(
        ...
        dom_to_lamports(20.0)
    );
    assert_eq!(
        client.get_balance(&alice.pubkey()).unwrap(),
        original_alice_balance - dom_to_lamports(20.0)
    );
}
