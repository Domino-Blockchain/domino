use {
    ...
    domino_core::validator::{ValidatorConfig, ValidatorStartProgress},
    domino_download_utils::{download_snapshot_archive, DownloadProgressRecord},
    domino_genesis_utils::download_then_check_genesis_hash,
    domino_gossip::{
...
fn verify_reachable_ports(
    ...
    domino_net_utils::verify_reachable_ports(
...
fn check_vote_account(
    ...
    let vote_state = domino_vote_program::vote_state::VoteState::from(&vote_account);
...
mod without_incremental_snapshots {
    ...
    pub fn rpc_bootstrap(
        ...
        loop {
            ...
            Ok(rpc_version) => {
                info!("RPC node version: {}", rpc_version.domino_core);
                Ok(())
            }
        ...
        .map(|_| {
            if !validator_config.voting_disabled && !bootstrap_config.no_check_vote_account {
                ...
                .unwrap_or_else(|err| {
                    ...
                    // incorrect `domino-validator` command-line arguments) rather than the
                    // RPC node failing.
                    //
