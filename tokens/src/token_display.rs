use {
    domino_account_decoder::parse_token::real_number_string_trimmed,
    domino_sdk::native_token::lamports_to_dom,
    std::{
...
impl Token {
    fn write_with_symbol(&self, f: &mut Formatter) -> Result {
        match &self.token_type {
            TokenType::Sol => {
                let amount = lamports_to_dom(self.amount);
                write!(f, "{}{}", DOMI_SYMBOL, amount)
            }
            TokenType::SplToken => {
