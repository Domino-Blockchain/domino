use {
    ...
    domino_account_decoder::parse_token::UiTokenAccount,
    domino_clap_utils::keypair::SignOnly,
    domino_client::rpc_response::{
    ...
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        for account in &self.accounts {
            writeln!(
                ...
                "{:<44}  {}",
                account.address,
                &format!("{} DOMI", lamports_to_dom(account.lamports))
            )?;
        }
        Ok(())
    }
...
impl fmt::Display for CliSupply {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        writeln_name_value(f, "Total:", &format!("{} DOMI", lamports_to_dom(self.total)))?;
        writeln_name_value(
            f,
            "Circulating:",
            &format!("{} DOMI", lamports_to_dom(self.circulating)),
        )?;
        writeln_name_value(
            f,
            "Non-Circulating:",
            &format!("{} DOMI", lamports_to_dom(self.non_circulating)),
        )?;
    ...
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        if !self.encoded_confirmed_block.rewards.is_empty() {
            ...
            writeln!(
                ...
                "Total Rewards: {}◎{:<12.9}",
                sign,
                lamports_to_dom(total_rewards.abs() as u64)
            )?;
        }
