...
source "$DOMINO_ROOT"/scripts/configure-metrics.sh
...
DOMINO_CONFIG_DIR=$DOMINO_ROOT/config
...
_setup_secondary_mount() {
  ...
  (
    ...
    if [[ -d $SECONDARY_DISK_MOUNT_POINT ]] && \
      ...
      rm -rf "$DOMINO_CONFIG_DIR"
      ln -sfT $SECONDARY_DISK_MOUNT_POINT/config "$DOMINO_CONFIG_DIR"
    fi
  )
}
