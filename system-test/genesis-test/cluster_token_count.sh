...
function get_cluster_version {
  clusterVersion="$(curl -s -X POST -H "Content-Type: application/json" -d '{"jsonrpc":"2.0","id":1, "method":"getVersion"}' "$url" | jq '.result | ."domino-core" ')"
...
function get_token_capitalization {
  ...
  totalSupplySol=$((totalSupplyLamports / LAMPORTS_PER_DOMI))
...
function get_program_account_balance_totals {
  ...
  totalAccountBalancesSol=$((totalAccountBalancesLamports / LAMPORTS_PER_DOMI))
