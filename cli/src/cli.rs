use {
    ...
    domino_clap_utils::{self, input_parsers::*, input_validators::*, keypair::*},
    domino_cli_output::{
    ...
    domino_vote_program::vote_state::VoteAuthorize,
...
pub enum CliError {
    ...
    #[error("Account {1} has insufficient funds for fee ({0} DOMI)")]
    InsufficientFundsForFee(f64, Pubkey),
    #[error("Account {1} has insufficient funds for spend ({0} DOMI)")]
...
impl CliConfig<'_> {
    fn default_keypair_path() -> String {
        domino_cli_config::Config::default().keypair_path
    }

    fn default_json_rpc_url() -> String {
        domino_cli_config::Config::default().json_rpc_url
    }

    fn default_websocket_url() -> String {
        domino_cli_config::Config::default().websocket_url
    }
    ...
    pub fn compute_websocket_url_setting(
        ...
        Self::first_nonempty_setting(vec![
            ...
            (
                ...
                domino_cli_config::Config::compute_websocket_url(&normalize_to_url_if_moniker(
                    json_rpc_cmd_url,
                )),
            ),
            (
    ...
    let response = match matches.subcommand() {
        ...
        ("completion", Some(matches)) => {
            ...
            get_clap_app(
                crate_name!(),
                crate_description!(),
                domino_version::version!(),
            )
            .gen_completions_to("domino", shell_choice, &mut stdout());
            std::process::exit(0);
        }
        // Cluster Query Commands
        ...
        //partkey Commands
        ("addpartkey", Some(matches)) => parse_addPartKey(matches, wallet_manager),
        ("listpartkeys",Some(matches)) => parse_listpartkey(matches,default_signer,wallet_manager),
        ("getpartkey", Some(matches)) => parse_getPartKey(matches, wallet_manager),
...
pub fn process_command(config: &CliConfig) -> ProcessResult {
    ...
    match &config.command {
        ...
        // Return software version of domino-cli and cluster entrypoint node
        CliCommand::Catchup {
            node_pubkey,
            node_json_rpc_url,
            follow,
        ...
        // Partkey Commands
        CliCommand::AddPartkey{pubkey} => {
            process_genPartKey(pubkey)
        }
        ...
        CliCommand::GetPartkey{pubkey} => {
            process_getPartKey(pubkey)
        }
...
mod tests {
    ...
    fn test_generate_unique_signers() {
        ...
        assert_eq!(
            signer_info.index_of(Some(domino_sdk::pubkey::new_rand())),
            None
        );
    ...
    fn test_cli_parse_command() {
        ...
        let from_pubkey = Some(domino_sdk::pubkey::new_rand());
        let from_str = from_pubkey.unwrap().to_string();
        for (name, program_id) in &[
            ("STAKE", stake::program::id()),
            ("VOTE", domino_vote_program::id()),
    ...
    fn test_cli_process_command() {
        ...
        let new_authorized_pubkey = domino_sdk::pubkey::new_rand();
        ...
        let stake_account_pubkey = domino_sdk::pubkey::new_rand();
        ...
        let stake_account_pubkey = domino_sdk::pubkey::new_rand();
        let source_stake_account_pubkey = domino_sdk::pubkey::new_rand();
