...
impl Pubkey {
    ...
    /// as a Domino `Pubkey`, except they are ensured to not be on the ed25519
    ...
    ///
    /// As all account addresses accessed by an on-chain Domino program must be
    ...
    /// program, which invokes an on-chain Domino program that uses the address
    ...
    /// instruction as a Domino transaction; and off-chain, as a library, so
    /// that clients can share the instruction data structure, constructors, and
    /// other common code.
    ///
    /// First the on-chain Domino program:
    ///
    /// ```
    ...
    ///
    /// ```ignore
    /// # // NB: This example depends on domino_sdk and domino_client, and adding
    /// # // those as dev-dependencies would create an unpublishable circular
    /// # // dependency, hence it is ignored.
    /// #
    /// # use borsh::{BorshSerialize, BorshDeserialize};
    /// # use domino_program::pubkey::Pubkey;
    /// # use domino_program::instruction::Instruction;
    /// # use domino_program::hash::Hash;
    /// # use domino_program::instruction::AccountMeta;
    /// # use domino_program::system_program;
    /// # use domino_sdk::signature::Keypair;
    /// # use domino_sdk::signature::{Signer, Signature};
    /// # use domino_sdk::transaction::Transaction;
    ...
    pub fn try_find_program_address(seeds: &[&[u8]], program_id: &Pubkey) -> Option<(Pubkey, u8)> {
        ...
        {
            extern "C" {
                fn dom_try_find_program_address(
                    seeds_addr: *const u8,
                    seeds_len: u64,
    ...
    pub fn log(&self) {
        ...
        {
            extern "C" {
                fn dom_log_pubkey(pubkey_addr: *const u8);
            }
            unsafe { dom_log_pubkey(self.as_ref() as *const _ as *const u8) };
        }

        #[cfg(not(target_arch = "bpf"))]
        crate::program_stubs::dom_log(&self.to_string());
    }
}
