...
[Staking and Inflation FAQ](https://dominochain.com/staking).

------

After you have [received DOMI](transfer-tokens.md), you might consider putting
it to use by delegating _stake_ to a validator. Stake is what we call tokens
in a _stake account_. Domino weights validator votes by the amount of stake
delegated to them, which gives those validators more influence in determining
then next valid block of transactions in the blockchain. Domino then generates
new DOMI periodically to reward stakers and validators. You earn more rewards
...
```bash
domino-keygen new --no-passphrase -o stake-account.json
```
...
```bash
domino create-stake-account --from <KEYPAIR> stake-account.json <AMOUNT> \
...
View the new stake account with the `domino stake-account` command:
...
`domino stake-authorize` command. For example, to set a new stake authority,
run:

```bash
domino stake-authorize <STAKE_ACCOUNT_ADDRESS> \
...
```bash
domino create-stake-account --from <KEYPAIR> <STAKE_ACCOUNT_KEYPAIR> --seed <STRING> <AMOUNT> \
...
and seed string. To see what stake address the command will derive, use `domino create-address-with-seed`:

```bash
domino create-address-with-seed --from <PUBKEY> <SEED_STRING> STAKE
```

`<PUBKEY>` is the public key of the `<STAKE_ACCOUNT_KEYPAIR>` passed to
`domino create-stake-account`.
...
accounts with the `domino validators` command:
...
address in `domino delegate-stake`:

```bash
domino delegate-stake --stake-authority <KEYPAIR> <STAKE_ACCOUNT_ADDRESS> <VOTE_ACCOUNT_ADDRESS> \
    --fee-payer <KEYPAIR>
```
...
After delegating stake, use `domino stake-account` to observe the changes
...
Delegated Stake: 4999.99771712 DOMI
...
Once delegated, you can undelegate stake with the `domino deactivate-stake`
command:

```bash
domino deactivate-stake --stake-authority <KEYPAIR> <STAKE_ACCOUNT_ADDRESS> \
    --fee-payer <KEYPAIR>
```
...
Transfer tokens out of a stake account with the `domino withdraw-stake` command:

```bash
domino withdraw-stake --withdraw-authority <KEYPAIR> <STAKE_ACCOUNT_ADDRESS> <RECIPIENT_ADDRESS> <AMOUNT> \
    --fee-payer <KEYPAIR>
```
...
existing stake account to a new one, use the `domino split-stake` command:

```bash
domino split-stake --stake-authority <KEYPAIR> <STAKE_ACCOUNT_ADDRESS> <NEW_STAKE_ACCOUNT_KEYPAIR> <AMOUNT> \
    --fee-payer <KEYPAIR>
```
