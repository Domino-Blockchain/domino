 ...
 */
#include <domi/sha.h>
#include <domi/keccak.h>
#include <domi/blake3.h>
...
extern uint64_t entrypoint(const uint8_t *input) {
  ...
  // SHA256
  {
    ...
    const SolBytes bytes[] = {{bytes1, DOMI_ARRAY_SIZE(bytes1)},
                              {bytes2, DOMI_ARRAY_SIZE(bytes2)}};

    dom_sha256(bytes, DOMI_ARRAY_SIZE(bytes), result);

    dom_assert(0 == dom_memcmp(result, expected, SHA256_RESULT_LENGTH));
  }
  ...
  // Keccak
  {
    ...
    dom_keccak256(bytes, DOMI_ARRAY_SIZE(bytes), result);

    dom_assert(0 == dom_memcmp(result, expected, KECCAK_RESULT_LENGTH));
  }
  ...
  // Blake3
  {
    ...
    dom_blake3(bytes, DOMI_ARRAY_SIZE(bytes), result);

    dom_assert(0 == dom_memcmp(result, expected, BLAKE3_RESULT_LENGTH));
  }
