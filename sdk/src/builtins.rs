    ...
    ($name:ident, $id:path, $entrypoint:expr) => {
        ...
        macro_rules! $name {
            () => {
                ...
                // see https://github.com/Domino-Blockchain/domino/issues/10933.
                // For now, we need to use `::domino_sdk`
                //
                // `respan!` respans the path `$crate::id`, which we then call (hence the extra
                // parens)
                (
                    stringify!($name).to_string(),
                    ::domino_sdk::respan!($crate::$id, $name)(),
                    $entrypoint,
                )
            };
        }
...
/// mod item_wrapper {
/// use domino_sdk::keyed_account::KeyedAccount;
/// use domino_sdk::instruction::InstructionError;
/// use domino_sdk::pubkey::Pubkey;
/// use domino_sdk::declare_builtin;
///
...
///
/// # use item_wrapper::id;
/// assert_eq!(id(), domino_sdk::system_program::ID);
/// ```
#[macro_export]
