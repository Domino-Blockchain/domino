...
Domino programs run on-chain, so debugging them in the wild can be challenging.
To make debugging programs easier, developers can write unit tests that directly
test their program's execution via the Domino runtime, or run a local cluster
...
`export RUST_LOG=domino_runtime::system_instruction_processor=trace,domino_runtime::message_processor=info,domino_bpf_loader=debug,domino_rbpf=debug`
...
- The BPF loader may fail to setup the program's execution environment
  - `InstructionError::Custom(0x0b9f_0001)` will be returned as part of the
    ...
    [`VirtualMachineCreationFailed`](https://github.com/domino-labs/domino/blob/bc7133d7526a041d1aaee807b80922baa89b6f90/programs/bpf_loader/src/lib.rs#L44).
- The BPF loader may have detected a fatal error during program executions
  ...
  - `InstructionError::Custom(0x0b9f_0002)` will be returned as part of the
    ...
    [`VirtualMachineFailedToRunProgram`](https://github.com/domino-labs/domino/blob/bc7133d7526a041d1aaee807b80922baa89b6f90/programs/bpf_loader/src/lib.rs#L46).
- The program itself may return an error
  - `InstructionError::Custom(<user defined value>)` will be returned. The
    ...
    program
    errors](https://github.com/domino-labs/domino/blob/bc7133d7526a041d1aaee807b80922baa89b6f90/sdk/program/src/program_error.rs#L87).
...
`domino_rbpf` level in `RUST_LOG` to `trace`. For example:

`export RUST_LOG=domino_rbpf=trace`
