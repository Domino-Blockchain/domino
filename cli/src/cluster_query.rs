use {
    ...
    domino_transaction_status::UiTransactionEncoding,
...
impl ClusterQuerySubCommands for App<'_, '_> {
    fn cluster_query_subcommands(self) -> Self {
        ...
        .subcommand(
            SubCommand::with_name("supply").about("Get information about the cluster supply of DOMI")
            .arg(
        ...
        .subcommand(
            SubCommand::with_name("total-supply").about("Get total number of DOMI")
...
pub fn process_catchup(
    ...
    loop {
        ...
        // especially if pointing to api.meinnet-beta.dominochain.com at times
...
pub fn process_total_supply(rpc_client: &RpcClient, _config: &CliConfig) -> ProcessResult {
    ...
    Ok(format!("{} DOMI", lamports_to_dom(supply.total)))
}
...
pub fn process_show_stakes(
    ...
    let mut program_accounts_config = RpcProgramAccountsConfig {
        account_config: RpcAccountInfoConfig {
            encoding: Some(domino_account_decoder::UiAccountEncoding::Base64),
...
mod tests {
    use {
        ...
        domino_sdk::signature::{write_keypair, Keypair},
        std::str::FromStr,
