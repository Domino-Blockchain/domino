use {
    ...
    regex::Regex,
    domino_download_utils::download_file,
    domino_sdk::signature::{write_keypair_file, Keypair},
    std::{
...
fn install_if_missing(
    ...
    // Check whether the package is already in ~/.cache/domino.
...
fn build_bpf_package(config: &Config, target_directory: &Path, package: &cargo_metadata::Package) {
    ...
    let legacy_program_feature_present = package.name == "domino-sdk";
    ...
    let bpf_tools_download_file_name = if cfg!(target_os = "macos") {
        "domino-bpf-tools-osx.tar.bz2"
    } else {
        "domino-bpf-tools-linux.tar.bz2"
    };
    ...
    if let Some(program_name) = program_name {
        ...
        println!("  $ domino program deploy {}", program_so.display());
...
fn main() {
    if cfg!(windows) {
        println!("Domino Rust BPF toolchain is not available on Windows");
        exit(1);
    }
    ...
    let matches = App::new(crate_name!())
        ...
        .arg(
            Arg::with_name("bpf_sdk")
                ...
                .help("Path to the Domino BPF SDK"),
        )
        .arg(
