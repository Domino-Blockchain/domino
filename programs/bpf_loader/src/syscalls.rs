...
pub fn register_syscalls(
    ...
    syscall_registry.register_syscall_by_name(b"dom_panic_", SyscallPanic::call)?;
    syscall_registry.register_syscall_by_name(b"dom_log_", SyscallLog::call)?;
    syscall_registry.register_syscall_by_name(b"dom_log_64_", SyscallLogU64::call)?;

    syscall_registry
        .register_syscall_by_name(b"dom_log_compute_units_", SyscallLogBpfComputeUnits::call)?;

    syscall_registry.register_syscall_by_name(b"dom_log_pubkey", SyscallLogPubkey::call)?;

    syscall_registry.register_syscall_by_name(
        b"dom_create_program_address",
        SyscallCreateProgramAddress::call,
    )?;
    syscall_registry.register_syscall_by_name(
        b"dom_try_find_program_address",
        SyscallTryFindProgramAddress::call,
    )?;

    syscall_registry.register_syscall_by_name(b"dom_sha256", SyscallSha256::call)?;
    syscall_registry.register_syscall_by_name(b"dom_keccak256", SyscallKeccak256::call)?;
    ...
    {
        syscall_registry
            .register_syscall_by_name(b"dom_secp256k1_recover", SyscallSecp256k1Recover::call)?;
    }
    ...
    {
        syscall_registry.register_syscall_by_name(b"dom_blake3", SyscallBlake3::call)?;
    }

    syscall_registry
        .register_syscall_by_name(b"dom_get_clock_sysvar", SyscallGetClockSysvar::call)?;
    syscall_registry.register_syscall_by_name(
        b"dom_get_epoch_schedule_sysvar",
    ...
    {
        syscall_registry
            .register_syscall_by_name(b"dom_get_fees_sysvar", SyscallGetFeesSysvar::call)?;
    }
    syscall_registry
        .register_syscall_by_name(b"dom_get_rent_sysvar", SyscallGetRentSysvar::call)?;

    syscall_registry.register_syscall_by_name(b"dom_memcpy_", SyscallMemcpy::call)?;
    syscall_registry.register_syscall_by_name(b"dom_memmove_", SyscallMemmove::call)?;
    syscall_registry.register_syscall_by_name(b"dom_memcmp_", SyscallMemcmp::call)?;
    syscall_registry.register_syscall_by_name(b"dom_memset_", SyscallMemset::call)?;

    // Cross-program invocation syscalls
    syscall_registry
        .register_syscall_by_name(b"dom_invoke_signed_c", SyscallInvokeSignedC::call)?;
    syscall_registry
        .register_syscall_by_name(b"dom_invoke_signed_rust", SyscallInvokeSignedRust::call)?;

    // Memory allocator
    syscall_registry.register_syscall_by_name(b"dom_alloc_free_", SyscallAllocFree::call)?;
    ...
    {
        syscall_registry
            .register_syscall_by_name(b"dom_set_return_data", SyscallSetReturnData::call)?;
        syscall_registry
            .register_syscall_by_name(b"dom_get_return_data", SyscallGetReturnData::call)?;
    }

    // Log data
    if invoke_context
        .feature_set
        .is_active(&dom_log_data_syscall_enabled::id())
    {
        syscall_registry.register_syscall_by_name(b"dom_log_data", SyscallLogData::call)?;
    }
...
pub fn bind_syscall_context_objects<'a, 'b>(
    ...
    let is_return_data_syscall_active = invoke_context
        .feature_set
        .is_active(&return_data_syscall_enabled::id());
    let is_dom_log_data_syscall_active = invoke_context
        .feature_set
        .is_active(&dom_log_data_syscall_enabled::id());
    ...
    bind_feature_gated_syscall_context_object!(
        vm,
        is_dom_log_data_syscall_active,
...
/// Panic syscall function, called when the BPF program calls 'dom_panic_()`
/// Causes the BPF program to be halted immediately
...
/// `dom_alloc_free_()`.  The allocator is expected to allocate/free
...
mod tests {
    ...
    fn test_translate_type() {
        ...
        let instruction = Instruction::new_with_bincode(
            ...
            &"foobar",
            vec![AccountMeta::new(domino_sdk::pubkey::new_rand(), false)],
        );
    ...
    fn test_translate_slice() {
        ...
        // Pubkeys
        let mut data = vec![domino_sdk::pubkey::new_rand(); 5];
        let addr = data.as_ptr() as *const _ as u64;
        let memory_mapping = MemoryMapping::new::<UserError>(
            vec![
                MemoryRegion::default(),
        ...
        .unwrap();
        assert_eq!(data, translated_data);
        data[0] = domino_sdk::pubkey::new_rand(); // Both should point to same place
        assert_eq!(data, translated_data);
    }
    ...
    fn test_syscall_dom_log() {
        ...
        invoke_context
            .push(&message, &message.instructions[0], &[0], &[])
            .unwrap();
        let mut syscall_dom_log = SyscallLog {
            invoke_context: Rc::new(RefCell::new(&mut invoke_context)),
        };
    ...
    fn test_syscall_dom_log_u64() {
        ...
        invoke_context
            ...
            .unwrap();
        let cost = invoke_context.get_compute_budget().log_64_units;
        let mut syscall_dom_log_u64 = SyscallLogU64 {
            invoke_context: Rc::new(RefCell::new(&mut invoke_context)),
        };
        ...
        let mut result: Result<u64, EbpfError<BpfError>> = Ok(0);
        syscall_dom_log_u64.call(1, 2, 3, 4, 5, &memory_mapping, &mut result);
        result.unwrap();
    ...
    fn test_syscall_dom_pubkey() {
        ...
        invoke_context
            ...
            .unwrap();
        let cost = invoke_context.get_compute_budget().log_pubkey_units;
        let mut syscall_dom_pubkey = SyscallLogPubkey {
            invoke_context: Rc::new(RefCell::new(&mut invoke_context)),
        };
        ...
        let mut result: Result<u64, EbpfError<BpfError>> = Ok(0);
        syscall_dom_pubkey.call(100, 32, 0, 0, 0, &memory_mapping, &mut result);
        assert_eq!(
            Err(EbpfError::UserError(BpfError::SyscallError(
                SyscallError::InstructionError(InstructionError::ComputationalBudgetExceeded)
            ))),
        ...
        syscall_dom_pubkey
            ...
            .mock_set_remaining(cost);
        let mut result: Result<u64, EbpfError<BpfError>> = Ok(0);
        syscall_dom_pubkey.call(0x100000000, 0, 0, 0, 0, &memory_mapping, &mut result);
        result.unwrap();
    ...
    #[test]
    fn test_syscall_dom_alloc_free() {
        let config = Config::default();
        // large alloc
        {
    ...
    fn test_create_program_address() {
        // These tests duplicate the direct tests in domino_program::pubkey
