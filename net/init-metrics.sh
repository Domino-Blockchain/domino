...
Creates a testnet dev metrics database
  ...
  -e              Assume database already exists and DOMINO_METRICS_CONFIG is
...
if $useEnv; then
  [[ -n $DOMINO_METRICS_CONFIG ]] ||
    usage "DOMINO_METRICS_CONFIG is not defined in the environment"
else
  username=$1
  ...
  DOMINO_METRICS_CONFIG="host=$host,db=$netBasename,u=scratch_writer,p=topsecret"
fi

echo "export DOMINO_METRICS_CONFIG=\"$DOMINO_METRICS_CONFIG\"" >> "$configFile"
