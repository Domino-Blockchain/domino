...
impl StakesCache {
    ...
    pub fn is_stake(account: &AccountSharedData) -> bool {
        domino_vote_program::check_id(account.owner())
    ...
    pub fn check_and_store(&self, pubkey: &Pubkey, account: &AccountSharedData) {
        if domino_vote_program::check_id(account.owner()) {
        ...
        } else if domino_stake_program::check_id(account.owner()) {
...
pub mod tests {
    use {
        ...
        domino_sdk::{account::WritableAccount, pubkey::Pubkey, rent::Rent},
        domino_stake_program::stake_state,
        domino_vote_program::vote_state::{self, VoteState, VoteStateVersions},
    };
    ...
    pub fn create_staked_node_accounts(
        ...
        let vote_account =
            vote_state::create_account(&vote_pubkey, &domino_sdk::pubkey::new_rand(), 0, 1);
        (
