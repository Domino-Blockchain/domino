#!/usr/bin/env bash
#
# Fetches the latest SPL programs and produces the domino-genesis command-line
...
fetch_program() {
  ...
  if [[ -r ~/.cache/domino-spl/$so ]]; then
    cp ~/.cache/domino-spl/"$so" "$so"
  else
    ...
    (
      ...
      curl -L --retry 5 --retry-delay 2 --retry-connrefused \
        -o "$so" \
        "https://github.com/Domino-Blockchain/domino-program-library/releases/download/$name-v$version/$so_name"
    )

    mkdir -p ~/.cache/domino-spl
    cp "$so" ~/.cache/domino-spl/"$so"
  fi
...
echo
echo "domino-genesis command-line arguments (spl-genesis-args.sh):"
