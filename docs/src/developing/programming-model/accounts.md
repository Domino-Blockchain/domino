...
In the same way that a Linux user uses a path to look up a file, a Domino client
...
Accordingly, the Domino SDK [checks the sysvar account's validity during
deserialization](https://github.com/Domino-Blockchain/domino/blob/a95675a7ce1651f7b59443eb146b356bc4b3f374/sdk/program/src/sysvar/mod.rs#L65).
A alternative and safer way to read a sysvar is via the sysvar's [`get()`
function](https://github.com/Domino-Blockchain/domino/blob/64bfc14a75671e4ec3fe969ded01a599645080eb/sdk/program/src/sysvar/mod.rs#L73)
...
Keeping accounts alive on Domino incurs a storage cost called _rent_ because the
...
This value is calculated to target 0.01 DOMI per mebibyte-day (exactly matching
to 3.56 DOMI per mebibyte-year):

```text
Rent fee: 19.055441478439427 = 10_000_000 (0.01 DOMI) * 365(approx. day in a year) / (1024 * 1024)(1 MiB) / (365.25/2)(epochs in 1 year)
```
...
Rent can also be estimated via the [`domino rent` CLI subcommand](cli/usage.md#domino-rent)
