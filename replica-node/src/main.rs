...
use {
    ...
    domino_runtime::accounts_index::AccountSecondaryIndexes,
    domino_sdk::{exit::Exit, pubkey::Pubkey, signature::Signer},
    domino_streamer::socket::SocketAddrSpace,
    domino_validator::port_range_validator,
    std::{
        collections::HashSet,
        env,
...
pub fn main() {
    ...
    let rpc_bind_address = if let Some(rpc_bind_address) = matches.value_of("rpc_bind_address") {
        domino_net_utils::parse_host(rpc_bind_address).expect("invalid rpc_bind_address")
    } else {
        bind_address
    };
    ...
    let peer_address = domino_net_utils::parse_host(matches.value_of("peer_address").unwrap())
    ...
    let logfile = {
        let logfile = matches
            .value_of("logfile")
            .map(|s| s.into())
            .unwrap_or_else(|| format!("domino-replica-node-{}.log", identity_keypair.pubkey()));
    ...
    let _logger_thread = domino_validator::redirect_stderr_to_file(logfile);
