...
### Primary Validator
The following additional `domino-validator` parameters are required to enable
...
following `domino-validator` command-line argument changes:
...
```bash
$ domino-validator wait-for-restart-window --identity validator-keypair.json \
  && domino-validator set-identity validator-keypair.json
```
...
Monitoring of your choosing can invoke the `domino-validator set-identity
