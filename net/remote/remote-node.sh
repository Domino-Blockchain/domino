...
cat >> ~/domino/on-reboot <<EOF
  ...
  sudo RUST_LOG=info ~domino/.cargo/bin/domino-sys-tuner --user $(whoami) > sys-tuner.log 2>&1 &
  echo \$! > sys-tuner.pid

  (
    sudo DOMINO_METRICS_CONFIG="$DOMINO_METRICS_CONFIG" scripts/oom-monitor.sh
  ) > oom-monitor.log 2>&1 &
  ...
  if ${GPU_CUDA_OK} && [[ -e /dev/nvidia0 ]]; then
    echo Selecting domino-validator-cuda
    export DOMINO_CUDA=1
  elif ${GPU_FAIL_IF_NONE} ; then
    echo "Expected GPU, found none!"
    export DOMINO_GPU_MISSING=1
  fi
EOF
  ...
  bootstrap-validator)
    ...
    if [[ $skipSetup != true ]]; then
      clear_config_dir "$DOMINO_CONFIG_DIR"
      ...
      setupValidatorKeypair() {
        ...
        if [[ -f net/keypairs/"$name".json ]]; then
          ...
          domino-keygen new --no-passphrase -so config/"$name".json
          if [[ "$name" =~ ^validator-identity- ]]; then
            name="${name//-identity-/-vote-}"
            domino-keygen new --no-passphrase -so config/"$name".json
            name="${name//-vote-/-stake-}"
        ...
        if [[ -n $internalNodesLamports ]]; then
          declare pubkey
          pubkey="$(domino-keygen pubkey config/"$name".json)"
...
EOF
      ...
      for i in $(seq 0 $((numBenchTpsClients-1))); do
        ...
        domino-bench-tps --write-client-keys config/bench-tps"$i".yml \
      ...
      if [[ "$extraPrimordialStakes" -gt 0 ]]; then
        ...
        for i in $(seq "$extraPrimordialStakes"); do
          args+=(--bootstrap-validator "$(domino-keygen pubkey "config/validator-identity-$i.json")"
                                       "$(domino-keygen pubkey "config/validator-vote-$i.json")"
                                       "$(domino-keygen pubkey "config/validator-stake-$i.json")"
          )
        done
      fi
      ...
      if [[ -n "$maybeWarpSlot" ]]; then
        ...
        domino-ledger-tool -l config/bootstrap-validator create-snapshot 0 config/bootstrap-validator $maybeWarpSlot
      fi

      domino-ledger-tool -l config/bootstrap-validator shred-version --max-genesis-archive-unpacked-size 1073741824 | tee config/shred-version

      if [[ -n "$maybeWaitForSupermajority" ]]; then
        bankHash=$(domino-ledger-tool -l config/bootstrap-validator bank-hash)
    ...
    if [[ "$tmpfsAccounts" = "true" ]]; then
      args+=(--accounts /mnt/domino-accounts)
    fi
...
EOF
  ...
  validator|blockstreamer)
    ...
    if [[ $skipSetup != true ]]; then
      ...
      if [[ $nodeType = blockstreamer ]]; then
        net/scripts/rsync-retry.sh -vPrc \
          "$entrypointIp":~/domino/config/blockstreamer-identity.json "$DOMINO_CONFIG_DIR"/validator-identity.json
      else
        net/scripts/rsync-retry.sh -vPrc \
          "$entrypointIp":~/domino/config/validator-identity-"$nodeIndex".json "$DOMINO_CONFIG_DIR"/validator-identity.json
        net/scripts/rsync-retry.sh -vPrc \
          "$entrypointIp":~/domino/config/validator-stake-"$nodeIndex".json "$DOMINO_CONFIG_DIR"/stake-account.json
        net/scripts/rsync-retry.sh -vPrc \
          "$entrypointIp":~/domino/config/validator-vote-"$nodeIndex".json "$DOMINO_CONFIG_DIR"/vote-account.json
      fi
      net/scripts/rsync-retry.sh -vPrc \
        "$entrypointIp":~/domino/config/shred-version "$DOMINO_CONFIG_DIR"/shred-version

      net/scripts/rsync-retry.sh -vPrc \
        "$entrypointIp":~/domino/config/bank-hash "$DOMINO_CONFIG_DIR"/bank-hash || true

      net/scripts/rsync-retry.sh -vPrc \
        "$entrypointIp":~/domino/config/faucet.json "$DOMINO_CONFIG_DIR"/faucet.json
    fi
    ...
    args=(
      ...
      --gossip-port 8001
      --rpc-port 8899
      --expected-shred-version "$(cat "$DOMINO_CONFIG_DIR"/shred-version)"
    )
    if [[ $nodeType = blockstreamer ]]; then
      args+=(
        --blockstream /tmp/domino-blockstream.sock
        --no-voting
        --dev-no-sigverify
    ...
    if [[ ! -f "$DOMINO_CONFIG_DIR"/validator-identity.json ]]; then
      domino-keygen new --no-passphrase -so "$DOMINO_CONFIG_DIR"/validator-identity.json
    fi
    args+=(--identity "$DOMINO_CONFIG_DIR"/validator-identity.json)
    if [[ ! -f "$DOMINO_CONFIG_DIR"/vote-account.json ]]; then
      domino-keygen new --no-passphrase -so "$DOMINO_CONFIG_DIR"/vote-account.json
    fi
    args+=(--vote-account "$DOMINO_CONFIG_DIR"/vote-account.json)
    ...
    if [[ -r "$DOMINO_CONFIG_DIR"/bank-hash ]]; then
      args+=(--expected-bank-hash "$(cat "$DOMINO_CONFIG_DIR"/bank-hash)")
    fi
    ...
    scp "$entrypointIp":~/domino/config/faucet.json "$DOMINO_CONFIG_DIR"/
    if [[ $nodeType = blockstreamer ]]; then
...
EOF
    ...
    if [[ $skipSetup != true && $nodeType != blockstreamer && -z $maybeSkipAccountsCreation ]]; then
      ...
      domino --url http://"$entrypointIp":8899 catchup config/validator-identity.json
      ...
      if [[ ${extraPrimordialStakes} -eq 0 ]]; then
        ...
        multinode-demo/delegate-stake.sh --vote-account "$DOMINO_CONFIG_DIR"/vote-account.json \
                                         --stake-account "$DOMINO_CONFIG_DIR"/stake-account.json \
