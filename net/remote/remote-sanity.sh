...
local|tar|skip)
  ...
  domino_cli=domino
  domino_gossip=domino-gossip
  domino_install=domino-install
  ;;
*)
...
(
  set -x
  $domino_cli --url http://"$sanityTargetIp":8899 validators
)
...
(
  ...
  $domino_gossip --allow-private-addr spy --entrypoint "$sanityTargetIp:8001" \
...
if $installCheck && [[ -r update_manifest_keypair.json ]]; then
  echo "--- $sanityTargetIp: domino-install test"
