...
use {
    ...
    domino_cli_output::display::format_labeled_address,
    domino_client::{client_error, rpc_client::RpcClient, rpc_response::RpcVoteAccountStatus},
    domino_metrics::{datapoint_error, datapoint_info},
    domino_notifier::Notifier,
    domino_sdk::{
        hash::Hash,
        native_token::{dom_to_lamports, Sol},
        pubkey::Pubkey,
    },
    std::{
        collections::HashMap,
...
fn get_config() -> Config {
    let matches = App::new(crate_name!())
        ...
        define environment variables before running `domino-watchtower`:
        ...
        define environment variable before running `domino-watchtower`:
        ...
        .arg(
            Arg::with_name("minimum_validator_identity_balance")
                ...
                .help("Alert when the validator identity balance is less than this amount of DOMI")
        )
        .arg(
    ...
    let minimum_validator_identity_balance = dom_to_lamports(value_t_or_exit!(
        matches,
...
fn main() -> Result<(), Box<dyn error::Error>> {
    ...
    domino_metrics::set_panic_hook("watchtower");
    ...
    loop {
        ...
        if let Some((failure_test_name, failure_error_message)) = &failure {
            ...
            if !last_notification_msg.is_empty() {
                ...
                info!("{}", all_clear_msg);
                notifier.send(&format!("domino-watchtower: {}", all_clear_msg));
            }
