...
lazy_static! {
    ...
    pub static ref CONFIG_FILE: Option<String> = {
        dirs_next::home_dir().map(|mut path| {
            path.extend(&[".config", "domino", "cli", "config.yml"]);
            path.to_str().unwrap().to_string()
        })
    };
}

/// The Domino CLI configuration.
#[derive(Serialize, Deserialize, Debug, PartialEq)]
pub struct Config {
    /// The RPC address of a Domino validator node.
    ///
    /// Typical values for mainnet, devnet, and testnet are [described in the
    /// Domino documentation][rpcdocs].
    ///
    ...
    /// documentation for `domino_clap_utils::keypair::signer_from_path`.
    ...
    /// function, or with `domino_clap_utils::keypair::DefaultSigner`.
    pub keypair_path: String,
    /// A mapping from Domino addresses to human-readable names.
    ///
    ...
    /// `domino_sdk::commitment_config::CommitmentLevel::Confirmed`.
    #[serde(default)]
