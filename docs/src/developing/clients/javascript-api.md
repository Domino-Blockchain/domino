...
```javascript
// dominoWeb3 is provided in the global namespace by the bundle script
console.log(dominoWeb3);
```
...
To allow users to use your dApp or application on Domino, they will need to get access to their Keypair. A Keypair is a private key with a matching public key, used to sign transactions.
...
To interact with programs on Domino, you create, sign, and send transactions to the network. Transactions are collections of instructions with signatures. The order that instructions exist in a transaction determines the order they are executed.

A transaction in Domino-Web3.js is created using the [`Transaction`](javascript-api.md#Transaction) object and adding desired messages, addresses, or instructions.

Take the example of a transfer transaction:

```javascript
const {Keypair, Transaction, SystemProgram, LAMPORTS_PER_DOMI} = require("@domino/web3.js");
...
```javascript
const {sendAndConfirmTransaction, clusterApiUrl, Connection} = require("@domino/web3.js");
...
The above code takes in a `TransactionInstruction` using `SystemProgram`, creates a `Transaction`, and sends it over the network. You use `Connection` in order to define with Domino network you are connecting to, namely `mainnet-beta`, `testnet`, or `devnet`.

### Interacting with Custom Programs

The previous section visits sending basic transactions. In Domino everything you do interacts with different programs, including the previous section's transfer transaction. At the time of writing programs on Domino are either written in Rust or C.

Let's look at the `SystemProgram`. The method signature for allocating space in your account on Domino in Rust looks like this:
...
In Domino when you want to interact with a program you must first know all the accounts you will be interacting with.
...
Let's look at how to call this instruction using domino-web3.js:
...
The above is created using using u32 and ns64 from `@domino/buffer-layout` to facilitate the payload creation. The `allocate` function takes in the parameter `space`. To interact with the function we must provide the data as a Buffer format. The `buffer-layout` library helps with allocating the buffer and encoding it correctly for Rust programs on Domino to interpret.
...
```javascript
const {struct, u32, ns64} = require("@domino/buffer-layout");
