...
declare -r DOMINO_LOCK_FILE="/home/domino/.domino.lock"
...
colo_instance_run() {
  ...
  declare OUT
  set +e
  OUT=$(ssh -l domino -o "StrictHostKeyChecking=no" -o "ConnectTimeout=3" -n "${IP}" "${CMD}" 2>&1)
  declare RC=$?
  set -e
  while read -r LINE; do
...
colo_whoami() {
  declare ME LINE DOMI_USER EOL
  while read -r LINE; do
    declare IP RC
    IFS=$'\x1f' read -r IP RC DOMI_USER EOL <<< "${LINE}"
    if [ "${RC}" -eq 0 ]; then
      [[ "${EOL}" = "EOL" ]] || echo "${FUNCNAME[0]}: Unexpected input \"${LINE}\"" 1>&2
      if [ -z "${ME}" ] || [ "${ME}" = "${DOMI_USER}" ]; then
        ME="${DOMI_USER}"
      else
        echo "Found conflicting username \"${DOMI_USER}\" on ${IP}, expected \"${ME}\"" 1>&2
      fi
    fi
  done < <(colo_instance_run_foreach "[ -n \"\${DOMINO_USER}\" ] && echo -e \"\${DOMINO_USER}\\x1fEOL\"")
  echo "${ME}"
}

COLO_DOMINO_USER=""
colo_get_domino_user() {
  if [ -z "${COLO_DOMINO_USER}" ]; then
    COLO_DOMINO_USER=$(colo_whoami)
  fi
  echo "${COLO_DOMINO_USER}"
}
...
__colo_node_status_script() {
  ...
  exec 2>/dev/null  # Suppress stderr as the next call to exec fails most of
                    # the time due to ${DOMINO_LOCK_FILE} not existing and is running from a
                    # subshell where normal redirection doesn't work
  exec 9<"${DOMINO_LOCK_FILE}" && flock -s 9 && . "${DOMINO_LOCK_FILE}" && exec 9>&-
  echo -e "\${DOMINO_LOCK_USER}\\x1f\${DOMINO_LOCK_INSTANCENAME}\\x1f\${PREEMPTIBLE}\\x1fEOL"
...
DOMINO_LOCK_FILE="${DOMINO_LOCK_FILE}"
INSTANCE_NAME="${INSTANCE_NAME}"
PREEMPTIBLE="${PREEMPTIBLE}"
SSH_AUTHORIZED_KEYS='$("${__colo_here}"/add-datacenter-domino-user-authorized_keys.sh 2> /dev/null)'
