...
case $testName in
test-stable)
  _ "$cargo" stable test --jobs "$NPROC" --all --exclude domino-local-cluster ${V:+--verbose} -- --nocapture
  ;;
test-stable-bpf)
  ...
  # domino-keygen required when building C programs
  ...
  # BPF domino-sdk legacy compile test
...
test-stable-perf)
  if [[ $(uname) = Linux ]]; then
    ...
    # Force CUDA for domino-core unit tests
  ...
  _ "$cargo" stable test --package domino-perf --package domino-ledger --package domino-core --lib ${V:+--verbose} -- --nocapture
  _ "$cargo" stable run --manifest-path poh-bench/Cargo.toml ${V:+--verbose} -- --hashes-per-tick 10
  ;;
test-local-cluster)
  _ "$cargo" stable build --release --bins ${V:+--verbose}
  _ "$cargo" stable test --release --package domino-local-cluster ${V:+--verbose} -- --nocapture --test-threads=1
  exit 0
  ;;
test-wasm)
  _ node --version
