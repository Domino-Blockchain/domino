use {
    ...
    log::*,
    domino_clap_utils::input_parsers::{lamports_of_dom, value_of},
    domino_faucet::{
        faucet::{run_faucet, Faucet, FAUCET_PORT},
        socketaddr,
    },
    domino_sdk::signature::read_keypair_file,
    std::{
        collections::HashSet,
...
#[tokio::main]
async fn main() {
    let default_keypair = domino_cli_config::Config::default().keypair_path;

    domino_logger::setup_with_default("domino=info");
    domino_metrics::set_panic_hook("faucet");
    let matches = App::new(crate_name!())
        ...
        .arg(
            Arg::with_name("per_time_cap")
                ...
                .value_name("NUM")
                .takes_value(true)
                .help("Request limit for time slice, in DOMI"),
        )
        .arg(
            Arg::with_name("per_request_cap")
                ...
                .value_name("NUM")
                .takes_value(true)
                .help("Request limit for a single request, in DOMI"),
        )
        .arg(
    ...
    let per_time_cap = lamports_of_dom(&matches, "per_time_cap");
    let per_request_cap = lamports_of_dom(&matches, "per_request_cap");
