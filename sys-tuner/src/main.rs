...
fn tune_poh_service_priority(uid: u32) {
    ...
    if let Some(pid) = find_pid("domino-validato", "/proc", uid, |dir| {
        let mut path = dir.path();
        path.push("task");
        find_pid("domino-poh-serv", path, uid, |dir1| {
...
fn main() {
    ...
    if let Err(e) = std::fs::remove_file(domino_sys_tuner::DOMINO_SYS_TUNER_PATH) {
        if e.kind() != std::io::ErrorKind::NotFound {
            panic!("Failed to remove stale socket file: {:?}", e)
        }
    }

    let listener = unix_socket::UnixListener::bind(domino_sys_tuner::DOMINO_SYS_TUNER_PATH)
    ...
    if let Some(user) = users::get_user_by_name(&user) {
        peer_uid = user.uid();
        info!("UID for domino is {}", peer_uid);
        nix::unistd::chown(
            domino_sys_tuner::DOMINO_SYS_TUNER_PATH,
