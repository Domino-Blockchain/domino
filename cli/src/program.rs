use {
    ...
    log::*,
    domino_account_decoder::{UiAccountEncoding, UiDataSliceConfig},
    domino_bpf_loader_program::{syscalls::register_syscalls, BpfError, ThisInstructionMeter},
...
fn report_ephemeral_mnemonic(words: usize, mnemonic: bip39::Mnemonic) {
    ...
    eprintln!(
        "`domino-keygen recover` and the following {}-word seed phrase:",
        words
    );
    eprintln!("{}\n{}\n{}", divider, phrase, divider);
    eprintln!("To resume a deploy, pass the recovered keypair as the");
    eprintln!("[BUFFER_SIGNER] to `domino program deploy` or `domino write-buffer'.");
    eprintln!("Or to recover the account's lamports, pass it as the");
    eprintln!(
        "[BUFFER_ACCOUNT_ADDRESS] argument to `domino program close`.\n{}",
        divider
    );
}
...
mod tests {
    use {
        ...
        serde_json::Value,
        domino_cli_output::OutputFormat,
