...
pub fn lamports_of_dom(matches: &ArgMatches<'_>, name: &str) -> Option<u64> {
    value_of(matches, name).map(dom_to_lamports)
}
...
mod tests {
    use {
        super::*,
        clap::{App, Arg},
        domino_sdk::signature::write_keypair_file,
        std::fs,
    };
    ...
    #[test]
    fn test_lamports_of_dom() {
        let matches = app()
            .clone()
            .get_matches_from(vec!["test", "--single", "50"]);
        assert_eq!(lamports_of_dom(&matches, "single"), Some(50_000_000_000));
        assert_eq!(lamports_of_dom(&matches, "multiple"), None);
        let matches = app()
            .clone()
            .get_matches_from(vec!["test", "--single", "1.5"]);
        assert_eq!(lamports_of_dom(&matches, "single"), Some(1_500_000_000));
        assert_eq!(lamports_of_dom(&matches, "multiple"), None);
        let matches = app()
            .clone()
            .get_matches_from(vec!["test", "--single", "0.03"]);
        assert_eq!(lamports_of_dom(&matches, "single"), Some(30_000_000));
    }
}
