use {
    ...
    domino_config_program::{config_instruction, get_config_data, ConfigState},
    domino_sdk::{
        hash::{Hash, Hasher},
        message::Message,
        pubkey::Pubkey,
...
fn load_release_target(release_dir: &Path) -> Result<String, String> {
    ...
    version_yml.push("domino-release");
...
pub fn init(
    ...
    let config = {
        // Write new config file only if different, so that running |domino-install init|
...
pub fn info(config_file: &str, local_info_only: bool, eval: bool) -> Result<(), String> {
    ...
    if eval {
        ...
        config
            ...
            .and_then(|channel| {
                println!("DOMINO_INSTALL_ACTIVE_CHANNEL={}", channel,);
                Option::<String>::None
            });
        return Ok(());
    }
...
fn check_for_newer_github_release(
    ...
    while page == 1 || releases.len() == PER_PAGE {
        let url = reqwest::Url::parse_with_params(
            "https://api.github.com/repos/domino-labs/domino/releases",
            &[
...
pub fn init_or_update(config_file: &str, is_init: bool, check_only: bool) -> Result<bool, String> {
        ...
        match explicit_release {
            ...
            ExplicitRelease::Channel(release_channel) => {
                ...
                let current_release_version_yml =
                    release_dir.join("domino-release").join("version.yml");
    ...
    symlink_dir(
        release_dir.join("domino-release"),
