...
Domino supports several different types of wallets that can be used to interface
directly with the Domino command-line tools.

**If you are unfamiliar with using command line programs and just want to be able
to send and receive DOMI tokens, we recommend setting up a third-party
[App Wallet](apps.md)**.

To use a Command Line Wallet, you must first [install the Domino CLI tools](../cli/install-domino-cli-tools.md)
