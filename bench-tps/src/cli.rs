use {
    ...
    domino_faucet::faucet::FAUCET_PORT,
    domino_sdk::{
...
const NUM_LAMPORTS_PER_ACCOUNT_DEFAULT: u64 = domino_sdk::native_token::LAMPORTS_PER_DOMI;
...
pub fn extract_args(matches: &ArgMatches) -> Config {
    ...
    if let Some(addr) = matches.value_of("entrypoint") {
        args.entrypoint_addr = domino_net_utils::parse_host_port(addr).unwrap_or_else(|e| {
    ...
    if let Some(addr) = matches.value_of("faucet") {
        args.faucet_addr = domino_net_utils::parse_host_port(addr).unwrap_or_else(|e| {
