use {
    ...
    domino_account_decoder::parse_token::{
    ...
    domino_sdk::{
        ...
        message::Message,
        native_token::{lamports_to_dom, dom_to_lamports},
    ...
    for allocation in allocations.iter() {
        ...
        let do_create_associated_token_account = if let Some(spl_token_args) = &args.spl_token_args
        {
            ...
            println!(
                "{:<44}  {:>24.9}",
                allocation.recipient,
                lamports_to_dom(allocation.amount)
            );
            false
        };
...
pub fn process_allocations(
    ...
    let starting_total_tokens = if let Some(spl_token_args) = &args.spl_token_args {
        ...
        Token::domi(starting_total_tokens)
    };
    println!(
        "{} {}",
    ...
    let (distributed_tokens, undistributed_tokens) =
        if let Some(spl_token_args) = &args.spl_token_args {
            ...
            (
                Token::domi(distributed_tokens),
                Token::domi(undistributed_tokens),
            )
        };
...
fn check_payer_balances(
    ...
    if let Some((unlocked_dom_source, total_unlocked_dom)) = unlocked_dom_source {
        ...
        if fee_payer_balance < fees + undistributed_tokens {
            return Err(Error::InsufficientFunds(
                vec![FundingSource::SystemAccount, FundingSource::FeePayer].into(),
                lamports_to_dom(fees + undistributed_tokens).to_string(),
            ));
        }
    } else {
        if fee_payer_balance < fees {
...
pub fn process_balances(client: &RpcClient, args: &BalancesArgs) -> Result<(), Error> {
    ...
    for allocation in &allocations {
        if let Some(spl_token_args) = &args.spl_token_args {
            ...
            let expected = lamports_to_dom(allocation.amount);
            let actual = lamports_to_dom(client.get_balance(&address).unwrap());
            println!(
                "{:<44}  {:>24.9}  {:>24.9}  {:>24.9}",
                allocation.recipient,
                expected,
...
use {
    ...
    domino_sdk::{pubkey::Pubkey, signature::Keypair},
...
pub fn test_process_distribute_tokens_with_client(
    ...
    let expected_amount = if let Some(amount) = transfer_amount {
        ...
        dom_to_lamports(1000.0)
    };
    let alice_pubkey = domino_sdk::pubkey::new_rand();
    ...
    wtr.write_record(&[
        ...
        lamports_to_dom(expected_amount).to_string(),
    ])
    .unwrap();
    wtr.flush().unwrap();
...
pub fn test_process_create_stake_with_client(client: &RpcClient, sender_keypair: Keypair) {
    ...
    let expected_amount = dom_to_lamports(1000.0);
    let alice_pubkey = domino_sdk::pubkey::new_rand();
    ...
    assert_eq!(
        ...
        expected_amount - dom_to_lamports(1.0),
    );
...
mod tests {
    use {
        ...
        domino_transaction_status::TransactionConfirmationStatus,
    };
    ...
    fn test_process_transfer_amount_allocations() {
        ...
        let client = RpcClient::new_with_commitment(url, CommitmentConfig::processed());
        test_process_distribute_tokens_with_client(&client, alice, Some(dom_to_lamports(1.5)));
    }
    ...
    fn test_read_allocations() {
        ...
        let allocation_domi = Allocation {
            recipient: alice_pubkey.to_string(),
            amount: dom_to_lamports(42.0),
            lockup_date: "".to_string(),
        };
    ...
    fn test_read_allocations_transfer_amount() {
        ...
        let amount = dom_to_lamports(1.5);
    ...
    fn test_apply_previous_transactions() {
        let alice = domino_sdk::pubkey::new_rand();
        let bob = domino_sdk::pubkey::new_rand();
        let mut allocations = vec![
            Allocation {
                recipient: alice.to_string(),
                amount: dom_to_lamports(1.0),
                lockup_date: "".to_string(),
            },
            Allocation {
    ...
    fn test_set_split_stake_lockup() {
        ...
        let stake_account_address = domino_sdk::pubkey::new_rand();
        let new_stake_account_address = domino_sdk::pubkey::new_rand();
    ...
    fn initialize_check_payer_balances_inputs(
        ...
        let recipient = domino_sdk::pubkey::new_rand();
        let allocations = vec![Allocation {
    ...
    fn test_check_payer_balances_distribute_tokens_single_payer() {
        let fees = 10_000;
        let fees_in_domi = lamports_to_dom(fees);
    ...
    fn test_check_payer_balances_distribute_stakes_single_payer() {
        ...
        let expensive_allocations = vec![Allocation {
            recipient: domino_sdk::pubkey::new_rand().to_string(),
            amount: dom_to_lamports(expensive_allocation_amount),
            lockup_date: "".to_string(),
        }];
    ...
    fn test_build_messages_dump_db() {
        ...
        let amount = dom_to_lamports(1.0);
