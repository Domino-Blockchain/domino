...
if [[ ! -f update_manifest_keypair.json ]]; then
  "$DOMINO_ROOT"/scripts/domino-install-update-manifest-keypair.sh "$OS"
fi
...
PATH="$DOMINO_ROOT"/target/debug:$PATH

set -x
# shellcheck disable=SC2086 # Don't want to double quote $maybeKeypair
balance=$(domino $maybeKeypair --url "$URL" balance --lamports)
if [[ $balance = "0 lamports" ]]; then
  # shellcheck disable=SC2086 # Don't want to double quote $maybeKeypair
  domino $maybeKeypair --url "$URL" airdrop 0.000000042
fi

# shellcheck disable=SC2086 # Don't want to double quote $maybeKeypair
domino-install deploy $maybeKeypair --url "$URL" "$DOWNLOAD_URL" update_manifest_keypair.json
