...
fn init(name: &OsStr) {
    ...
    unsafe {
        INIT_HOOK.call_once(|| {
            let path;
            let lib_name = if let Some(perf_libs_path) = domino_perf::perf_libs::locate_perf_libs()
            {
                domino_perf::perf_libs::append_to_ld_library_path(
...
impl EntrySlice for [Entry] {
    ...
    fn verify_cpu_x86_simd(&self, start_hash: &Hash, simd_len: usize) -> EntryVerificationState {
        use domino_sdk::hash::HASH_BYTES;
        let now = Instant::now();
        let genesis = [Entry {
            num_hashes: 0,
            hash: *start_hash,
...
mod tests {
    use {
        super::*,
        domino_perf::test_tx::{test_invalid_tx, test_tx},
        domino_sdk::{
            hash::{hash, Hash},
            pubkey::Pubkey,
