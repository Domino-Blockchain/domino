...
#     git clone https://github.com/Domino-Blockchain/testnet-keypairs.git "${REPO_ROOT}"/net/keypairs
    git clone git@github.com:domino-labs/testnet-keypairs.git "${REPO_ROOT}"/net/keypairs
  ...
  get_net_launch_software_version_launch_args "$CHANNEL" "domino-release" version_args
...
[[ -n $INFLUX_HOST ]] || INFLUX_HOST=https://metrics.dominochain.com:8086
...
if [[ -z $DOMINO_METRICS_CONFIG ]]; then
  if [[ -z $DOMINO_METRICS_PARTIAL_CONFIG ]]; then
    echo DOMINO_METRICS_PARTIAL_CONFIG not defined
    exit 1
  fi
  export DOMINO_METRICS_CONFIG="db=$TESTNET_TAG,host=$INFLUX_HOST,$DOMINO_METRICS_PARTIAL_CONFIG"
fi
echo "DOMINO_METRICS_CONFIG: $DOMINO_METRICS_CONFIG"
