//! Communication with a Domino node over RPC.
//!
//! Software that interacts with the Domino blockchain, whether querying its
//! state or submitting transactions, communicates with a Domino node over
...
/// A client of a remote Domino node.
///
/// `RpcClient` communicates with a Domino node over [JSON-RPC], with the
/// [Domino JSON-RPC protocol][jsonprot]. It is the primary Rust interface for
...
///
/// ```
/// # use domino_sdk::system_transaction;
/// # use domino_client::rpc_client::RpcClient;
/// # use domino_client::client_error::ClientError;
/// # use domino_sdk::signature::{Keypair, Signer};
/// # use domino_sdk::hash::Hash;
...
impl RpcClient {
    ...
    ///
    /// ```
    /// # use domino_sdk::commitment_config::CommitmentConfig;
    /// # use domino_client::rpc_client::RpcClient;
    /// let url = "http://localhost:8899".to_string();
    ...
    fn get_node_version(&self) -> Result<semver::Version, RpcError> {
        ...
        if let Some(version) = &*r_node_version {
            ...
            let node_version = semver::Version::parse(&node_version.domino_core).map_err(|e| {
    ...
    ///
    /// [`TransactionError`]: domino_sdk::transaction::TransactionError
    ///
    /// This function only searches a node's recent history, including all
    /// recent slots, plus up to
    /// [`MAX_RECENT_BLOCKHASHES`][domino_sdk::clock::MAX_RECENT_BLOCKHASHES]
    ...
    /// #     client_error::ClientError,
    /// # };
    /// # use domino_sdk::slot_history::Slot;
    /// # let rpc_client = RpcClient::new_mock("succeeds".to_string());
    /// let start_slot = 1;
    /// let limit = 3;
    ...
    ///
    /// ```
    /// # use domino_transaction_status::UiTransactionEncoding;
    /// # use domino_client::{
    /// #     rpc_client::RpcClient,
    /// #     client_error::ClientError,
    /// # };
    ...
    /// #     rpc_client::RpcClient,
    /// # };
    /// # use domino_client::rpc_config::RpcLeaderScheduleConfig;
    /// # use domino_sdk::commitment_config::CommitmentConfig;
    /// # let rpc_client = RpcClient::new_mock("succeeds".to_string());
    /// # let slot = rpc_client.get_slot()?;
    ...
    /// Returns the current domino version running on the node.
    ///
    /// # RPC Reference
    ///
    ...
    /// let version = semver::Version::parse(&version.domino_core)?;
    ...
    /// #     commitment_config::CommitmentConfig,
    /// # };
    /// # use domino_account_decoder::UiAccountEncoding;
    /// # let rpc_client = RpcClient::new_mock("succeeds".to_string());
    /// # let alice = Keypair::new();
    /// # let bob = Keypair::new();
    /// let pubkeys = vec![alice.pubkey(), bob.pubkey()];
    ...
    /// #     commitment_config::CommitmentConfig,
    /// # };
    /// # use domino_account_decoder::{UiDataSliceConfig, UiAccountEncoding};
    /// # let rpc_client = RpcClient::new_mock("succeeds".to_string());
    /// # let alice = Keypair::new();
    /// # let base64_bytes = "\
