//! The domino-program-test provides a BanksClient-based test framework BPF programs
...
use {
    ...
    log::*,
    domino_banks_client::start_client,
    domino_banks_server::banks_server::start_local_server,
    domino_program_runtime::{ic_msg, invoke_context::ProcessInstructionWithContext, stable_log},
    domino_runtime::{
...
// Export types so test clients can limit their domino crate dependencies
pub use {domino_banks_client::BanksClient, domino_program_runtime::invoke_context::InvokeContext};
...
pub fn builtin_process_instruction(
    process_instruction: domino_sdk::entrypoint::ProcessInstruction,
...
/// Converts a `domino-program`-style entrypoint into the runtime's entrypoint style, for
    ...
    ($process_instruction:expr) => {
        Some(
            |first_instruction_account: usize,
             input: &[u8],
             invoke_context: &mut domino_program_test::InvokeContext| {
...
struct SyscallStubs {}
impl domino_sdk::program_stubs::SyscallStubs for SyscallStubs {
    ...
    fn dom_get_clock_sysvar(&self, var_addr: *mut u8) -> u64 {
        get_sysvar::<Clock>(&clock::id(), var_addr)
    }

    fn dom_get_epoch_schedule_sysvar(&self, var_addr: *mut u8) -> u64 {
        get_sysvar::<EpochSchedule>(&epoch_schedule::id(), var_addr)
    }

    #[allow(deprecated)]
    fn dom_get_fees_sysvar(&self, var_addr: *mut u8) -> u64 {
        get_sysvar::<Fees>(&fees::id(), var_addr)
    }

    fn dom_get_rent_sysvar(&self, var_addr: *mut u8) -> u64 {
...
impl ProgramTest {
        ...
        let add_bpf = |this: &mut ProgramTest, program_file: PathBuf| {
            ...
            this.add_account(
                ...
                Account {
                    ...
                    data,
                    owner: domino_sdk::bpf_loader::id(),
                    executable: true,
                    rent_epoch: 0,
                },
            );
    ...
    fn setup_bank(
    ...
    ) {
        {
            ...
            ONCE.call_once(|| {
                domino_sdk::program_stubs::set_syscall_stubs(Box::new(SyscallStubs {}));
            });
        }
        ...
        let bootstrap_validator_stake_lamports =
            rent.minimum_balance(VoteState::size_of()) + dom_to_lamports(1_000_000.0);

        let mint_keypair = Keypair::new();
        let voting_keypair = Keypair::new();

        let mut genesis_config = create_genesis_config_with_leader_ex(
            dom_to_lamports(1_000_000.0),
            &mint_keypair.pubkey(),
        ...
        add_builtin!(domino_bpf_loader_deprecated_program!());
        if self.use_bpf_jit {
            add_builtin!(domino_bpf_loader_program_with_jit!());
            add_builtin!(domino_bpf_loader_upgradeable_program_with_jit!());
        } else {
            add_builtin!(domino_bpf_loader_program!());
            add_builtin!(domino_bpf_loader_upgradeable_program!());
        }
    ...
    /// with DOMI for sending transactions
...
impl ProgramTestContext {
    ...
    pub fn warp_to_slot(&mut self, warp_slot: Slot) -> Result<(), ProgramTestError> {
        ...
        bank_forks.set_root(
            pre_warp_slot,
            &domino_runtime::accounts_background_service::AbsRequestSender::default(),
            Some(pre_warp_slot),
        );
