use {
    ...
    domino_sdk::{
        clock::UnixTimestamp, hash::Hash, message::Message, native_token::lamports_to_dom,
        program_utils::limited_deserialize, pubkey::Pubkey, stake, transaction::Transaction,
    },
    domino_transaction_status::UiTransactionStatusMeta,
...
pub fn build_balance_message_with_config(
    ...
    let value = if config.use_lamports_unit {
        ...
        let domi = lamports_to_dom(lamports);
        let dom_str = format!("{:.9}", domi);
...
pub fn write_transaction<W: io::Write>(
    ...
    for (instruction_index, instruction) in message.instructions.iter().enumerate() {
        ...
        if program_pubkey == domino_vote_program::id() {
            if let Ok(vote_instruction) = limited_deserialize::<
                domino_vote_program::vote_instruction::VoteInstruction,
            >(&instruction.data)
            {
        ...
        } else if program_pubkey == domino_sdk::system_program::id() {
            if let Ok(system_instruction) = limited_deserialize::<
                domino_sdk::system_instruction::SystemInstruction,
            >(&instruction.data)
            {
    ...
    if let Some(transaction_status) = transaction_status {
        ...
        writeln!(
            ...
            "{}  Fee: ◎{}",
            prefix,
            lamports_to_dom(transaction_status.fee)
        )?;
        assert_eq!(
...
#[cfg(test)]
mod test {
    use {super::*, domino_sdk::pubkey::Pubkey};
