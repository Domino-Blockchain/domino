...
RELEASE_BASENAME="${RELEASE_BASENAME:=domino-release}"
...
(
  ...
  cp "${RELEASE_BASENAME}"/bin/domino-install-init domino-install-init-$TARGET
...
for file in "${TARBALL_BASENAME}"-$TARGET.tar.bz2 "${TARBALL_BASENAME}"-$TARGET.yml domino-install-init-"$TARGET"* $MAYBE_TARBALLS; do
