...
use {
    log::*,
    serial_test::serial,
    domino_core::validator::ValidatorConfig,
    domino_gossip::{cluster_info::Node, contact_info::ContactInfo},
    domino_local_cluster::{
        cluster::Cluster,
...
const RUST_LOG_FILTER: &str =
    "error,domino_core::replay_stage=warn,domino_local_cluster=info,local_cluster=info";
...
fn setup_snapshot_validator_config(
    ...
    let accountsdb_repl_port =
        domino_net_utils::find_available_port_in_range(bind_ip_addr, (1024, 65535)).unwrap();
...
fn test_replica_bootstrap() {
    ...
    domino_logger::setup_with_default(RUST_LOG_FILTER);
    ...
    let port = domino_net_utils::find_available_port_in_range(ip_addr, (8101, 8200)).unwrap();
    let rpc_addr = SocketAddr::new(ip_addr, port);

    let port = domino_net_utils::find_available_port_in_range(ip_addr, (8201, 8300)).unwrap();
    ...
    let port = domino_net_utils::find_available_port_in_range(ip_addr, (8301, 8400)).unwrap();
    let gossip_addr = SocketAddr::new(ip_addr, port);

    let dynamic_port_range = domino_net_utils::parse_port_range("8401-8500").unwrap();
    let bind_address = domino_net_utils::parse_host("127.0.0.1").unwrap();
