...
use {
    ...
    domino_ledger::{blockstore::create_new_ledger, create_new_tmp_ledger},
    domino_net_utils::PortRange,
...
impl TestValidatorGenesis {
    ...
    pub fn add_account_with_file_data(
        ...
        self.add_account(
            ...
            AccountSharedData::from(Account {
                lamports,
                data: domino_program_test::read_file(
                    domino_program_test::find_file(filename).unwrap_or_else(|| {
    ...
    pub fn add_program(&mut self, program_name: &str, program_id: Pubkey) -> &mut Self {
        let program_path = domino_program_test::find_file(&format!("{}.so", program_name))
...
impl TestValidator {
    ...
    fn initialize_ledger(
        ...
        let validator_identity_lamports = dom_to_lamports(500.);
        let validator_stake_lamports = dom_to_lamports(1_000_000.);
        let mint_lamports = dom_to_lamports(500_000_000.);

        let mut accounts = config.accounts.clone();
        for (address, account) in domino_program_test::programs::spl_programs(&config.rent) {
            accounts.entry(address).or_insert(account);
        }
        for program in &config.programs {
            let data = domino_program_test::read_file(&program.program_path);
            accounts.insert(
                program.program_id,
                AccountSharedData::from(Account {
        ...
        let mut genesis_config = create_genesis_config_with_leader_ex(
            ...
            config.rent,
            domino_sdk::genesis_config::ClusterType::Development,
        ...
        let ledger_path = match &config.ledger_path {
            ...
            Some(ledger_path) => {
                ...
                let _ = create_new_ledger(
                    ...
                    domino_ledger::blockstore_db::AccessType::PrimaryOnly,
                )
                .map_err(|err| {
                    format!(
    ...
    fn start(
        ...
        // Needed to avoid panics in `domino-responder-gossip` in tests that create a number of
