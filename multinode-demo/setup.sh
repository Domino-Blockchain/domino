...
if [[ -f $BOOTSTRAP_VALIDATOR_STAKE_KEYPAIR ]]; then
  cp -f "$BOOTSTRAP_VALIDATOR_STAKE_KEYPAIR" "$DOMINO_CONFIG_DIR"/bootstrap-validator/stake-account.json
else
...
if [[ -f $BOOTSTRAP_VALIDATOR_VOTE_KEYPAIR ]]; then
  cp -f "$BOOTSTRAP_VALIDATOR_VOTE_KEYPAIR" "$DOMINO_CONFIG_DIR"/bootstrap-validator/vote-account.json
else
...
"$DOMINO_ROOT"/fetch-spl.sh
if [[ -r spl-genesis-args.sh ]]; then
  SPL_GENESIS_ARGS=$(cat "$DOMINO_ROOT"/spl-genesis-args.sh)
  #shellcheck disable=SC2207
  #shellcheck disable=SC2206
  args+=($SPL_GENESIS_ARGS)
fi

default_arg --ledger "$DOMINO_CONFIG_DIR"/bootstrap-validator
default_arg --faucet-pubkey "$DOMINO_CONFIG_DIR"/faucet.json
