use {
    ...
    domino_core::tower_storage::FileTowerStorage,
    domino_faucet::faucet::{run_local_faucet_with_port, FAUCET_PORT},
    domino_rpc::rpc::JsonRpcConfig,
    domino_sdk::{
    ...
    domino_validator::{
        ...
        redirect_stderr_to_file, domino_test_validator::*,
    },
    std::{
        collections::HashSet,
        fs, io,
 ...
 * 40MB-150MB range when running `domino-test-validator`
 */
const DEFAULT_MAX_LEDGER_SHREDS: u64 = 10_000;

const DEFAULT_FAUCET_DOMI: f64 = 1_000_000.;
...
fn main() {
    ...
    let default_faucet_domi = DEFAULT_FAUCET_DOMI.to_string();

    let matches = App::new("domino-test-validator")
        ...
        .arg({
            ...
            if let Some(ref config_file) = *domino_cli_config::CONFIG_FILE {
        ...
        .arg(
            Arg::with_name("json_rpc_url")
                ...
                .help(
                    "URL for Domino's JSON RPC or moniker (or their first letter): \
    ...
    domino_runtime::snapshot_utils::remove_tmp_snapshot_archives(&ledger_path);
    ...
    let cli_config = if let Some(config_file) = matches.value_of("config_file") {
        domino_cli_config::Config::load(config_file).unwrap_or_default()
    } else {
        domino_cli_config::Config::default()
    };
    ...
    let dynamic_port_range = matches.value_of("dynamic_port_range").map(|port_range| {
        domino_net_utils::parse_port_range(port_range).unwrap_or_else(|| {
            eprintln!("Failed to parse --dynamic-port-range");
            exit(1);
        })
    });
    let bind_address = matches.value_of("bind_address").map(|bind_address| {
        domino_net_utils::parse_host(bind_address).unwrap_or_else(|err| {
            ...
            match address_program {
                [address, program] => {
                    ...
                    programs.push(ProgramInfo {
                        program_id: address,
                        loader: domino_sdk::bpf_loader::id(),
                        program_path,
                    });
                }
                _ => unreachable!(),
    ...
    let faucet_lamports = dom_to_lamports(value_of(&matches, "faucet_dom").unwrap());
    ...
    if TestValidatorGenesis::ledger_exists(&ledger_path) {
        ...
        println_name_value(
            "\nNotice!",
            "No wallet available. `domino airdrop` localnet DOMI after creating one\n",
        );
    }
