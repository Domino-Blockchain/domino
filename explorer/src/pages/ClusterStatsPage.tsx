...
import {
  ...
  usePerformanceInfo,
  useStatsProvider,
} from "providers/stats/dominoClusterStats";
...
function StakingComponent() {
  ...
  const coinInfo = useCoinGecko("domino");
          ...
          <div className="card-body">
              ...
              <>
                <h4>
                  ...
                  <span className="ms-2 badge bg-primary rank">
                    Rank #{dominoInfo.market_cap_rank}
                  </span>
                </h4>
                <h1>
                  <em>${dominoInfo.price.toFixed(2)}</em>{" "}
                  {dominoInfo.price_change_percentage_24h > 0 && (
                    <small className="change-positive">
                      &uarr; {dominoInfo.price_change_percentage_24h.toFixed(2)}
                      %
                    </small>
                  )}
                  {dominoInfo.price_change_percentage_24h < 0 && (
                    <small className="change-negative">
                      &darr; {dominoInfo.price_change_percentage_24h.toFixed(2)}
                      %
                    </small>
                  )}
                  {dominoInfo.price_change_percentage_24h === 0 && (
                ...
                <h5>
                  24h Vol: <em>${abbreviatedNumber(dominoInfo.volume_24)}</em>{" "}
                  MCap: <em>${abbreviatedNumber(dominoInfo.market_cap)}</em>
                </h5>
              </>
            )}
            ...
            {dominoInfo && (
              <p className="updated-time text-muted">
                Updated at{" "}
                {displayTimestampWithoutDate(dominoInfo.last_updated.getTime())}
              </p>
            )}
          </div>
        </div>
