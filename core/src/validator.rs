...
pub use domino_perf::report_target_features;
use {
    crate::{
    ...
    rand::{thread_rng, Rng},
    domino_accountsdb_plugin_manager::accountsdb_plugin_service::AccountsDbPluginService,
    domino_entry::poh::compute_hash_time_ns,
    domino_gossip::{
        cluster_info::{
...
pub struct Validator {
    ...
    tpu: Tpu,
    tvu: Tvu,
    ip_echo_server: Option<domino_net_utils::IpEchoServer>,
...
impl Validator {
    ...
    pub fn new(
        ...
        if domino_perf::perf_libs::api().is_some() {
        ...
        *start_progress.write().unwrap() = ValidatorStartProgress::StartingServices;
        ...
        let ip_echo_server = match node.sockets.ip_echo {
            None => None,
            Some(tcp_listener) => Some(domino_net_utils::ip_echo_server(
                tcp_listener,
...
mod tests {
    use {
        super::*,
        domino_ledger::{create_new_tmp_ledger, genesis_utils::create_genesis_config_with_leader},
        domino_sdk::{genesis_config::create_genesis_config, poh_config::PohConfig},
    ...
    fn test_backup_and_clear_blockstore() {
        ...
        use {
            domino_entry::entry,
            domino_ledger::{blockstore, get_tmp_ledger_path},
        };
    ...
    fn test_poh_speed() {
        ...
        let poh_config = PohConfig {
            target_tick_duration: Duration::from_millis(domino_sdk::clock::MS_PER_TICK),
            // make PoH rate really fast to cause the panic condition
            hashes_per_tick: Some(100 * domino_sdk::clock::DEFAULT_HASHES_PER_TICK),
            ..PohConfig::default()
        };
