...
airdrops_enabled=1
node_dom=500 # 500 DOMI: number of DOMI to airdrop the node for transaction fees and vote account rent exemption (ignored if airdrops_enabled=0)
label=
identity=
vote_account=
no_restart=0
...
OPTIONS:
  ...
  --node-domi DOMI            - Number of DOMI this node has been funded from the genesis config (default: $node_dom)
...
if [[ "$DOMINO_GPU_MISSING" -eq 1 ]]; then
...
if [[ -z "$ledger_dir" ]]; then
  ledger_dir="$DOMINO_CONFIG_DIR/validator$label"
fi
mkdir -p "$ledger_dir"
...
if [[ -n $DOMINO_CUDA ]]; then
  program=$domino_validator_cuda
else
  program=$domino_validator
fi
...
wallet() {
  (
    set -x
    $domino_cli --keypair "$identity" --url "$rpc_url" "$@"
  )
}
  ...
  if ! wallet vote-account "$vote_account"; then
    if ((airdrops_enabled)); then
      ...
      (
        set -x
        $domino_cli \
          --keypair "$DOMINO_CONFIG_DIR/faucet.json" --url "$rpc_url" \
          transfer --allow-unfunded-recipient "$identity" "$node_dom"
      ) || return $?
    fi
...
rpc_url=$($domino_gossip $maybe_allow_private_addr rpc-url --timeout 180 --entrypoint "$gossip_entrypoint")

[[ -r "$identity" ]] || $domino_keygen new --no-passphrase -so "$identity"
[[ -r "$vote_account" ]] || $domino_keygen new --no-passphrase -so "$vote_account"
[[ -r "$authorized_withdrawer" ]] || $domino_keygen new --no-passphrase -so "$authorized_withdrawer"

setup_validator_accounts "$node_dom"
