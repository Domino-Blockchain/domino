...
- [`domino-accountsdb-plugin-interface`] &mdash; This crate defines the plugin
interfaces.

- [`domino-accountsdb-plugin-postgres`] &mdash; The crate for the referential
plugin implementation for the PostgreSQL database.

[`domino-accountsdb-plugin-interface`]: https://docs.rs/domino-accountsdb-plugin-interface
[`domino-accountsdb-plugin-postgres`]: https://docs.rs/domino-accountsdb-plugin-postgres


The Plugin Interface
====================

The Plugin interface is declared in [`domino-accountsdb-plugin-interface`]. It
...
The [`domino-accountsdb-plugin-postgres`] crate implements a plugin storing
...
```
{
	"libpath": "/domino/target/release/libdomino_accountsdb_plugin_postgres.so",
...
The sample [postgresql.conf](https://github.com/Domino-Blockchain/domino/blob/7ac43b16d2c766df61ae0a06d7aaf14ba61996ac/accountsdb-plugin-postgres/scripts/postgresql.conf)
...
Create the database. For example, the following creates a database named 'domino':

```
sudo -u postgres createdb domino -p 5433
```

Create the database user. For example, the following creates a regular user named 'domino':

```
sudo -u postgres createuser -p 5433 domino
```
...
```
psql -U domino -p 5433 -h 10.138.0.9 -w -d domino
```

### Create the Schema Objects

Use the [create_schema.sql](https://github.com/Domino-Blockchain/domino/blob/7ac43b16d2c766df61ae0a06d7aaf14ba61996ac/accountsdb-plugin-postgres/scripts/create_schema.sql)
to create the objects for storing accounts and slots.

Download the script from github:

```
wget https://raw.githubusercontent.com/domino-labs/domino/7ac43b16d2c766df61ae0a06d7aaf14ba61996ac/accountsdb-plugin-postgres/scripts/create_schema.sql
```

Then run the script:

```
psql -U domino -p 5433 -h 10.138.0.9 -w -d domino -f create_schema.sql
```
...
[drop_schema.sql](https://github.com/Domino-Blockchain/domino/blob/7ac43b16d2c766df61ae0a06d7aaf14ba61996ac/accountsdb-plugin-postgres/scripts/drop_schema.sql).
For example,

```
psql -U domino -p 5433 -h 10.138.0.9 -w -d domino -f drop_schema.sql
```
