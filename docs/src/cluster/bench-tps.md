...
```bash
./fetch-perf-libs.sh
NDEBUG=1 DOMINO_CUDA=1 ./multinode-demo/bootstrap-validator.sh
NDEBUG=1 DOMINO_CUDA=1 ./multinode-demo/validator.sh
```
...
- To enable `info` everywhere and `debug` only in the domino::banking_stage module:

  ```bash
  export RUST_LOG=domino=info,domino::banking_stage=debug
  ```

- To enable BPF program logging:

  ```bash
  export RUST_LOG=domino_bpf_loader=trace
  ```
...
You can also attach to a running process with GDB. The leader's process is named _domino-validator_:
