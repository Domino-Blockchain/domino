use {
    crate::client_error,
    domino_account_decoder::{parse_token::UiTokenAmount, UiAccount},
    domino_sdk::{
...
pub struct RpcVersionInfo {
    /// The current version of domino-core
    pub domino_core: String,
    /// first 4 bytes of the FeatureSet identifier
    pub feature_set: Option<u32>,
}
...
impl fmt::Debug for RpcVersionInfo {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(f, "{}", self.domino_core)
    }
}

impl fmt::Display for RpcVersionInfo {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        if let Some(version) = self.domino_core.split_whitespace().next() {
