...
use {
    ...
    domino_entry::poh::compute_hashes_per_tick,
    domino_genesis::{genesis_accounts::add_genesis_accounts, Base64Account},
    domino_ledger::{blockstore::create_new_ledger, blockstore_db::AccessType},
    ...
    domino_vote_program::vote_state::{self, VoteState},
    std::{
        collections::HashMap,
        error,
        fs::File,
...
fn main() -> Result<(), Box<dyn error::Error>> {
    let default_faucet_pubkey = domino_cli_config::Config::default().keypair_path;
    ...
    domino_stake_program::add_genesis_accounts(&mut genesis_config);
    if genesis_config.cluster_type == ClusterType::Development {
        domino_runtime::genesis_utils::activate_all_features(&mut genesis_config);
    }
...
mod tests {
    use {
        super::*,
        domino_sdk::genesis_config::GenesisConfig,
    ...
    fn test_append_primordial_accounts_to_genesis() {
        ...
        genesis_accounts.insert(
            domino_sdk::pubkey::new_rand().to_string(),
            Base64Account {
                owner: domino_sdk::pubkey::new_rand().to_string(),
                balance: 2,
                executable: false,
