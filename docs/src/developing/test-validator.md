---
title: Domino Test Validator
---

During early stage development, it is often convenient to target a cluster with
fewer restrictions and more configuration options than the public offerings
provide. This is easily achieved with the `domino-test-validator` binary, which
...
The `domino-test-validator` binary ships with the Domino CLI Tool Suite.
[Install](/cli/install-domino-cli-tools) before continuing.
...
Leave `domino-test-validator` running in its own terminal. When it is no longer
needed, it can be stopped with ctrl-c.

## Interacting

Open a new terminal to interact with a [running](#running) `domino-test-validator`
instance using other binaries from the Domino CLI Tool Suite or your own client
software.

#### Configure the CLI Tool Suite to target a local cluster by default

```
domino config set --url http://127.0.0.1:8899
```
...
- **NOTE:** The result should match the `Genesis Hash:` field in the
  `domino-test-validator` status output
...
- **NOTE:** `Error: No such file or directory (os error 2)` means that the default
  wallet does not yet exist. Create it with `domino-keygen new`.
...
```
domino transfer EPhgPANa5Rh2wa4V2jxt7YbtWa3Uyw4sTeZ13cQjDDB8 1
```
